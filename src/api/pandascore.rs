...
#[derive(Deserialize)]
struct PsOpponent { opponent: PsTeam }

#[derive(Deserialize)]
struct PsTeam { name: String, acronym: Option<String>, location: Option<String> }

#[derive(Deserialize)]
struct PsResult { score: u8 }

#[derive(Deserialize)]
struct PsLeague { name: String }

#[derive(Deserialize)]
struct PsStream { raw_url: Option<String> }
...
impl PandaScoreProvider {
    ...
    pub fn parse_matches(json: &str) -> Result<Vec<Match>, ApiError> {
        let ps_matches: Vec<PsMatch> = serde_json::from_str(json).map_err(|e| ApiError::Parse(e.to_string()))?;
        let mut matches = Vec::new();
        for m in ps_matches {
            if m.opponents.len() < 2 { continue; }
            ...
            let series_format = match m.number_of_games {
                2 => SeriesFormat::Bo2, 3 => SeriesFormat::Bo3,
                5 => SeriesFormat::Bo5, _ => SeriesFormat::Bo1,
            };

            let start_time = m.scheduled_at.as_deref()
                .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
                .map(|dt| dt.with_timezone(&Utc))
                .unwrap_or_else(Utc::now);

            let stream_url = m.streams_list.as_ref()
                .and_then(|s| s.first())
            ...
            matches.push(Match {
                id: m.id.to_string(),
                team_a: Team { name: team_a.name.clone(), tag: team_a.acronym.clone().unwrap_or_else(|| team_a.name.clone()), region: team_a.location.clone() },
                team_b: Team { name: team_b.name.clone(), tag: team_b.acronym.clone().unwrap_or_else(|| team_b.name.clone()), region: team_b.location.clone() },
                score_a, score_b, status, series_format,
                tournament_name: league_name.clone(),
                tournament_id: league_name.to_lowercase().replace(' ', "-"),
                start_time, stream_url, game_time_secs: None,
            });
        }
        Ok(matches)
    }
    ...
    pub fn parse_tournaments(json: &str) -> Result<Vec<Tournament>, ApiError> {
        let ps_tournaments: Vec<PsTournament> = serde_json::from_str(json).map_err(|e| ApiError::Parse(e.to_string()))?;
        let now = Utc::now();
        let mut tournaments = Vec::new();
        for t in ps_tournaments {
            let start_date = t.begin_at.as_deref()
                .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
                .map(|dt| dt.with_timezone(&Utc)).unwrap_or(now);
            let end_date = t.end_at.as_deref()
                .and_then(|s| DateTime::parse_from_rfc3339(s).ok())
                .map(|dt| dt.with_timezone(&Utc)).unwrap_or(now);
            ...
            let tier = match t.tier.as_deref() {
                Some("s") => "S-Tier", Some("a") => "A-Tier",
                Some("b") => "B-Tier", Some("c") => "C-Tier",
                Some(other) => other, None => "Unknown",
            }.to_string();

            tournaments.push(Tournament {
                id: t.id.to_string(), name: t.name, start_date, end_date,
                status, tier, location: None, prize_pool: t.prizepool,
            });
        }
        Ok(tournaments)
    }
